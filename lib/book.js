...
export async function getOrNullDraft(tripId) {
  const [d] = await q(
    "SELECT * FROM book_drafts WHERE trip_id=$1 ORDER BY id DESC LIMIT 1", [tripId]);
  return d || null;
}

export async function getDraftById(draftId) {
  const [d] = await q("SELECT * FROM book_drafts WHERE id=$1", [draftId]);
  return d || null;
}

export async function listDrafts(tripId) {
  return q(
    "SELECT id, name, mode, status, error, created_at, updated_at FROM book_drafts WHERE trip_id=$1 ORDER BY id DESC",
    [tripId]);
}

// Creates a new draft row synchronously; returns the new draftId immediately
// so the client can start polling. The AI work runs fire-and-forget.
export async function createDraftRow(tripId, mode) {
  const [row] = await q(
...
// Full generation into a pre-created draft row. Fire-and-forget.
export async function generateDraft(userId, tripId, mode, draftId) {
  const keepExcluded = [];
  try {
    ...
    const title = spec.title && spec.title !== "Untitled Book" ? spec.title : null;
    await q(
      `UPDATE book_drafts SET spec=$1, status='idle', error=NULL, updated_at=now()
       ${title ? ", name=$3" : ""} WHERE id=$2`,
      title ? [JSON.stringify(spec), draftId, title] : [JSON.stringify(spec), draftId]);
...
export async function aiEdit(userId, tripId, instruction, scope = {}, draftId) {
  const draft = draftId ? await getDraftById(draftId) : await getOrNullDraft(tripId);
...
export async function saveDraftSpec(tripId, spec, cutRevision, draftId) {
  const draft = draftId ? await getDraftById(draftId) : await getOrNullDraft(tripId);
  if (!draft) throw new Error("No draft exists");
  if (cutRevision) await saveRevision(draft.id, draft.spec, "manual", "manual edits");
  const title = spec.title && spec.title !== "Untitled Book" ? spec.title : null;
  await q(
    `UPDATE book_drafts SET spec=$1, updated_at=now() ${title ? ", name=$3" : ""} WHERE id=$2`,
    title ? [JSON.stringify(spec), draft.id, title] : [JSON.stringify(spec), draft.id]);
  return draft.id;
}
...
export async function scoreUserPhotos(userId) {
  ...
  const rows = await q(
    `SELECT DISTINCT p.id, coalesce(p.thumb_key, p.preview_key) AS score_key,
            p.ts, p.place_name
  ...
  try {
    for (const p of rows) {
      try {
        const img = await getObjectBuffer(p.score_key);
