...
export async function currentUser() {
  ...
  if (rows[0]) {
    // Last-active heartbeat, hourly granularity. Fire-and-forget so it
    // never adds latency or failure modes to the request itself.
    q(`UPDATE users SET last_active_at=now() WHERE id=$1
