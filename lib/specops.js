...
const TPL_COUNT = {
  ...
  // freestyle: photos tracked via pg.elements, slot count irrelevant
  "freestyle": 0,
};
  ...
  for (const ch of spec.chapters) for (const pg of ch.pages) {
    if (pg.template === "freestyle") {
      for (const el of pg.elements || [])
        if (el.type === "photo" && el.photoId) used.set(Number(el.photoId), pg.id);
    } else {
...
function validPage(page, validIds, excluded, usedElsewhere) {
  ...
  if (t === "freestyle") {
    // Freestyle: validate photo IDs inside elements
    for (const el of page.elements || []) {
      if (el.type !== "photo" || !el.photoId) continue;
      const id = Number(el.photoId);
...
export function validateSpec(spec, photoRows) {
  ...
  for (const ch of spec.chapters) {
    for (const pg of ch.pages) {
      ...
      else if (pg.template === "freestyle") {
        for (const el of pg.elements || [])
          if (el.type === "photo" && el.photoId) seen.set(Number(el.photoId), pg.id);
      } else {
        for (const id of pg.photoIds.map(Number)) seen.set(id, pg.id);
      }
    }
  }
  return errors;
...
export function repairSpec(spec, photoRows) {
  ...
  for (const ch of spec.chapters) {
    for (const pg of ch.pages) {
      if (pg.template === "freestyle") {
        // Freestyle: filter invalid photos from elements, leave positioning intact
        for (const el of pg.elements || []) {
          if (el.type === "photo" && el.photoId) {
            const id = Number(el.photoId);
            if (!validIds.has(id) || excluded.has(id) || seen.has(id)) el.photoId = null;
            else seen.add(id);
          }
        }
        continue;
    ...
    ch.pages = ch.pages.filter(pg =>
      ...
      pg.template !== "text-only" || (pg.text || "").trim()
    );
  }
...
export function applyOps(spec0, ops, photoRows) {
  ...
  // drop photo-less pages the ops may have created via exclusion (never drop freestyle)
  for (const ch of spec.chapters)
    ch.pages = ch.pages.filter(pg =>
      pg.template === "freestyle" ||
      pg.template !== "text-only" || (pg.text || "").trim() || pg.pinned
    );
